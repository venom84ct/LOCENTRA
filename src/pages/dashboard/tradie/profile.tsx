  ...
  const handleSave = async () => {
    ...
    if (avatarFile) {
      ...
      if (!error && data?.path) {
        ...
        updatedProfile.avatar_url = publicUrlData.publicUrl;
      }
    }

    // Upload portfolio files
    if (portfolioFiles) {
      const newPortfolio: string[] = [];
      ...
      for (const file of Array.from(portfolioFiles)) {
        ...
        if (!error && data?.path) {
          const { data: publicUrlData } = supabase.storage
            .from("portfolio")
            .getPublicUrl(data.path);
          newPortfolio.push(publicUrlData.publicUrl);
        }
      }

      updatedProfile.portfolio = [...(profile.portfolio || []), ...newPortfolio];
    }
